...
async function getCallMonitorData() {
    ...
    const result1 = await dbpool.query(`
        ...
        (
            ...
            from rc_csn a where DATE_ADD(UTC_TIMESTAMP(),interval -1 hour)<=a.eventTime
            group by a.sessionId ) as b on c.sequence=b.sequence and c.sessionId=b.sessionId
        where c.status='Proceeding' or c.status='Setup' or (c.status='Disconnected' and statusReason='BlindTransfer' ) and c.queue_name='General - Utility Switchboard' `
    );
