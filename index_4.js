...
async function insertAgentPresence(dataObj) {
    ...
    await dbpool.query(selectQuery, (err, res, cols) => {
        ...
        console.log(dataObj);
        CheckCallQueue(dataObj.extensionId, dataObj.presenceStatus, dataObj.userStatus, actionType, dataObj.telephonyStatus);
    });

}


function CheckCallQueue(extensionId, presenceStatus, userStatus, action, meetingStatus) {
    try {
        let IsQueueMember = 1;
        InsertUpdateAgentDb(extensionId, presenceStatus, action, IsQueueMember, userStatus, meetingStatus);
    }
    catch (e) {
        console.log(e);
        InsertUpdateAgentDb(extensionId, presenceStatus, action, 1, userStatus, meetingStatus);
    }

}

function InsertUpdateAgentDb(extensionId, presenceStatus, action, IsQueueMember, userStatus, meetingStatus) {
    if (action == "I") {
        const InsertQuery = `Insert into rc_agents(extensionId,presenceStatus,LastUpdated,IsQueueMember,userStatus,meetingStatus) values('${extensionId}','${presenceStatus}',CURDATE(),${IsQueueMember},'${userStatus}','${meetingStatus}')`
        ...
        const updateQuery = `update rc_agents set presenceStatus='${presenceStatus}',userStatus='${userStatus}',meetingStatus='${meetingStatus}',LastUpdated=CURDATE(),IsQueueMember=${IsQueueMember} where extensionId='${extensionId}' `;
...
async function getCallMonitorData() {
    ...
    var dbextList = [];
    const availSql = `select distinct extensionId from rc_agents where presenceStatus='Available' and userStatus='Available' and meetingStatus='NoCall' and IsQueueMember=1`;
