...
async function insertAgentPresence(dataObj) {
    ...
    const selectQuery = `select * from rc_agents where extensionId='${dataObj.extensionId}'`
    await dbpool.query(selectQuery, async (err, res, cols) => {
        if (err) console.log(err);

        if (res.length > 0) {
            await InsertUpDateAgent(dataObj.extensionId, dataObj.presenceStatus, "U")
        }
        else {
            await InsertUpDateAgent(dataObj.extensionId, dataObj.presenceStatus, "I")
        }
...
async function InsertUpDateAgent(extensionId, presenceStatus, action) {
    ...
    platform.get(`/restapi/v1.0/account/${process.env.RC_ACCOUNTID}/call-queues/${process.env.RC_GROUPID}/members`, queryParams).then(async (result) => {
        // PROCESS RESPONSE
        ...
        if (action == "I") {
            ...
            await dbpool.query(InsertQuery, (error, results, fields) => {
                if (error) console.log(error);
            ...
            await dbpool.query(updateQuery, (error, results, fields) => {
                if (error) console.log(error);
...
async function getCallMonitorData() {
    ...
    const availSql = `select distinct count(extensionId) as 'availableCount' from rc_agents where presenceStatus='Available' and IsQueueMember=1`;
