...
appInsights.setup("c8f9b103-5ec0-410b-8b16-e426c3e1d40c")
...
async function getCallMonitorData() {
    ...
    const availSql2 = `select Count(distinct extensionId) as 'availableCount' from rc_agents where presenceStatus='Available' and userStatus='Available' and meetingStatus='NoCall' and dndStatus='TakeAllCalls' and IsQueueMember=1 and GroupID2='${process.env.RC_GROUPID2}' `;
    const result2 = await dbpool.query(availSql2);
    if (result2.length > 0) {
        response.availableCount2 = result2[0].availableCount;
    }

    const availSql3 = `select Count(distinct extensionId) as 'availableCount' from rc_agents where presenceStatus='Available' and userStatus='Available' and meetingStatus='NoCall' and dndStatus='TakeAllCalls' and IsQueueMember=1 and GroupID3='${process.env.RC_GROUPID3}' `;
    const result3 = await dbpool.query(availSql3);
    if (result3.length > 0) {
        response.availableCount3 = result3[0].availableCount;
    }

    const onCallSql = `select Count(distinct extensionId) as 'availableCount' from rc_agents where presenceStatus='Busy' and userStatus='Available' and (meetingStatus='CallConnected' or meetingStatus='OnHold') and dndStatus='TakeAllCalls' and IsQueueMember=1 and GroupID1='${process.env.RC_GROUPID}' `;
    const resultOn = await dbpool.query(onCallSql);
    if (resultOn.length > 0) {
        response.OnCallCount1 = resultOn[0].availableCount;
    }

    const onCallSql2 = `select Count(distinct extensionId) as 'availableCount' from rc_agents where presenceStatus='Busy' and userStatus='Available' and (meetingStatus='CallConnected' or meetingStatus='OnHold') and dndStatus='TakeAllCalls'  and IsQueueMember=1 and GroupID2='${process.env.RC_GROUPID2}' `;
    const resultOn2 = await dbpool.query(onCallSql2);
    if (resultOn2.length > 0) {
        response.OnCallCount2 = resultOn2[0].availableCount;
    }

    const onCallSql3 = `select Count(distinct extensionId) as 'availableCount' from rc_agents where presenceStatus='Busy' and userStatus='Available' and (meetingStatus='CallConnected' or meetingStatus='OnHold') and dndStatus='TakeAllCalls' and IsQueueMember=1 and GroupID3='${process.env.RC_GROUPID3}' `;
    const resultOn3 = await dbpool.query(onCallSql3);
    if (resultOn3.length > 0) {
        response.OnCallCount3 = resultOn3[0].availableCount;
    }
    ...
    const resultQueue3 = await dbpool.query(`
        select count(*) as 'queueCount' from rc_csn c
        inner join 
        (
            select max(a.sequence) as 'sequence',a.sessionId 
            from rc_csn a where DATE_ADD(UTC_TIMESTAMP(),interval -1 hour)<=a.eventTime and a.queue_name='${process.env.RC_QUEUE3}'
            group by a.sessionId ) as b on c.sequence=b.sequence and c.sessionId=b.sessionId
        where c.status='Proceeding' or c.status='Setup' or (c.status='Disconnected' and statusReason='BlindTransfer' )  or (c.extensionId='undefined' and c.status='Answered')
        and c.queue_name='${process.env.RC_QUEUE3}' `
    );

    if (resultQueue3.length > 0) {
        response.queueCount3 = resultQueue3[0].queueCount;
    }
...
async function getSumMonitorData() {
    ...
    let response = {
        ...
        Sumofqueue3callsinlastminute: 0,
        OnCallCount1: 0,
        OnCallCount2: 0,
        OnCallCount3: 0
    };
    ...
    const resultQueue3 = await dbpool.query(`
            ...
            select min(a.eventtime) as 'eventtime',a.sessionId from rc_csn a
            where  
            a.queue_name='${process.env.RC_QUEUE3}'
            group by a.sessionId having  DATE_ADD(UTC_TIMESTAMP(),interval -30 minute)<=min(a.eventTime)
            ) as b where DATE_ADD(UTC_TIMESTAMP(),interval -60 second)<=b.eventTime
        `
    );
    ...
    if (resultQueue3.length > 0) {
        response.Sumofqueue3callsinlastminute = resultQueue3[0].queueCount;
    }
...
async function UpdateGroupID3(dbextList) {
    ...
    const result3 = await platform.get(`/restapi/v1.0/account/${process.env.RC_ACCOUNTID}/call-queues/${process.env.RC_GROUPID3}/members`, queryParams);
    var datalist = JSON.parse(result3._text);
    //console.log(datalist);
    ...
    if (datalist.records.length > 0) {
        ...
        dbextList.forEach(item => {
            var maplist = datalist.records.filter(m => m.id == item.extensionId).length;
            if (maplist > 0) {
                dbpool.query(`update rc_agents set GroupID3='${process.env.RC_GROUPID3}'  where extensionId='${item.extensionId}'`);
            }
            else {
                dbpool.query(`update rc_agents set GroupID3=''  where extensionId='${item.extensionId}'`);
            }
